...
public class StepDefs {
	...
	public void i_make_a_POST_request_to(String endpoint) {
		...
		gvm.setResponse(NavInitialization.api().POST_createDeepLinkingURL(endpoint, gvm.getBranchKey()));
	}
	
	@Given("I make a GET request with the android user-agent")
	public void i_make_a_GET_request_with_the_android_user_agent() throws ClientProtocolException, IOException {
		gvm.setResponse(NavInitialization.api().GET_responseURL(gvm.getResponseURL()));
	}

	@Then("It should return a {int} success response")
	public void it_should_return_a_success_response(Integer StatusCode) {
		NavInitialization.api().responseURLCheckStatus(StatusCode, gvm.getResponse());
	}
	...
	public void i_click_on_View_Status_using_Action_context() {
	    NavInitialization.DashboardPage().viewStatus();
	}
	...
	public void i_should_see_the_updated_android_click_count_in_the_stats() {
	    NavInitialization.DashboardPage().androidLinkClicks(gvm.getClickCount());
	}
