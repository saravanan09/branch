...
public class DashboardPageNav {
	private static final DashboardPage dashboardPage = PageFactory
			.initElements(BrowserUtils.getDriver(), DashboardPage.class);
	...
	public void viewClickEntry() {
		...
		if (dashboardPage.EventPlatform.getText().equals("ANDROID_WEB")) {
			Assert.assertEquals("CLICK", dashboardPage.EventName.getText());
			Assert.assertEquals("ANDROID_WEB",
					dashboardPage.EventPlatform.getText());
		}

	}

	public void viewStatus(String url) {
		BrowserUtils.WebPageLoader(5000);
		int rowCount = BrowserUtils
				.getDriver()
				.findElements(
						By.cssSelector("table tbody[data-testid='smart-table-body'] tr"))
				.size();
		...
		for (int i = 1; i <= rowCount; i++) {
			if (BrowserUtils
					.getDriver()
					.findElement(
							By.xpath("//tbody[@data-testid='smart-table-body']/tr["
									+ i + "]/td[3]/div/div/a"))
					.getAttribute("href").equals(url)) {
				Actions action = new Actions(BrowserUtils.getDriver());
				WebElement options = BrowserUtils.getDriver().findElement(
						By.xpath("//tbody[@data-testid='smart-table-body']/tr["
								+ i + "]/td[8]//button"));
				action.moveToElement(options).perform();
				WebElement viewStatus = BrowserUtils.getDriver().findElement(
						By.xpath("//tbody[@data-testid='smart-table-body']/tr["
								+ i
								+ "]/td[8]//a[@data-testid='la-view-stats']"));
	...
	public void androidLinkClicks(String expectedCount) {
		...
		Assert.assertEquals(Integer.parseInt(expectedCount) + 1,
				dashboardPage.AndroidClickCount.getText());
	}
